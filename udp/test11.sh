...
do
	...
	for (( i = 0; i < 10; i++ ))
	do
		rsh $RECEIVER_e /root/nfsrw/socket/udp/./receiver_control_bytes_UDP 1 $PORT $size 1 30000 | grep Band &
		sleep 0.5
		rsh $SENDER_e /root/nfsrw/socket/udp/./senderUDP $RECEIVER_g $PORT $size 1 30000 1 | grep Band
		echo ""
	done
	wait
done
