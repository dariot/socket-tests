...
int main(int argc, char *argv[])
{
	...
	if(argc < 6) {
		printf("\nUsage: ./receiver <num_sender> <first_receiver_port> <packet_size_in_bytes> <packet_number> <control_port>\n\n");
		exit(1);
	}
	...
	*/
	...
	for (k = 0; k < packet_number; k++) {
		for (j = 0; j < num_sender; j++) {
			//bytes_received = recv(connected[j], recv_data, message_size, MSG_WAITALL);
                        ...
                        bytes_received = recvfrom_large(connected[j], recv_data, message_size, 0, (struct sockaddr_in *) &sender_addr[k], &sin_size );
	...
	for (i = 0; i < num_sender; i++) {
		...
		sender_addr[i].sin_port = htons(control_port);
